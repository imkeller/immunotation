...
valid_organisms <- mro.obo$name[organism_children]
...
retrieve_MHC_proteincomplex <- function(alleles = allele_list,
    ...
    lookup <- assemble_lookup(mro.obo, organism_id = org_id,
                              level = "chain level")
...
retrieve_lookup_table <- function(organism) {
    # check if the organism name is valid
    org_id <- organism_input(organism)
    assemble_lookup(mro.obo, organism_id = org_id,
                    level = "chain level")
}
...
extract_mhc_type_table <- function(species_locus, obo_file, protein_masked_intersections) {
    all_species_loci <- get_descendants(obo_file, species_locus)
...
assemble_lookup <- function(obo_file, organism_id,
    ...
    protein_masked_intersections <- obo_file$intersection_of[grep("PR:000000001", obo_file$intersection_of)]
    
    tables <- sapply(species_loci, extract_mhc_type_table, obo_file = obo_file,
    ...
    mhc_type_names <-  obo_file$name[obo_file$id %in% unique(chain_table_filtered$mhc_type_id)]
    chain_table_filtered$mhc_type_names <- mhc_type_names[chain_table_filtered$mhc_type_id]
    
    # get names of locus
    locus_names <-  obo_file$name[obo_file$id %in% unique(chain_table_filtered$locus_id)]
    chain_table_filtered$locus_names <- locus_names[chain_table_filtered$locus_id]
    
    # get the names of chains
    chain_names <- obo_file$name[obo_file$id %in% chain_table_filtered$chain_id]
...
filter_molecules <- function(subcomplex) {
    subcomplex[grep("complete molecule", obo_file$property_value[subcomplex])]
}

#complex_list <- complexes_locus_level[[1]]
find_descendant_complexes <- function(complex_list) {
    subcomplexes <- sapply(complex_list, function(X) get_descendants(roots = X, ontology = obo_file, exclude_roots = TRUE))
    sapply(subcomplexes, filter_molecules )
}

extract_mhc_complex_table <- function(complex_sublist_list, mhc_type, obo_file = mro.obo) {
    ...
    if (length(complex_sublist) != 0) {
        ...
        complex_names <- obo_file$name[obo_file$id %in% complex_sublist]
        ...
        alpha_names_dict <- obo_file$name[obo_file$id %in% alpha_chains]
        alpha_names <- alpha_names_dict[alpha_chains]
        # beta chain
        beta_chains <- sapply(complex_chains, function(x) x[2])
        
        beta_names_dict <- obo_file$name[obo_file$id %in% beta_chains]
...
assemble_protein_complex <- function(obo_file = mro.obo, organism_id) {
    ...
    complexes_mhc_level <- obo_file$children[obo_file$id %in% c("MRO:0001355", "MRO:0001356", "MRO:0001464")]
    # filter for taxon of interest
    entries_for_species <- obo_file$id[grep(organism_id, obo_file$intersection_of)]
    ...
    complexes_locus_level <- obo_file$children[obo_file$id %in% species_complexes_mhc_level]
    ...
    complete_table <- data.frame(mhc_type = 0, complex_type = 0, complex_name = 0, complex_status = 0, alpha_name = 0,  beta_name = 0)
    # loop over mhc class
