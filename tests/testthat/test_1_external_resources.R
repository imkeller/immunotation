...
test_that("getURL() returns an error when the arguments are not matching", {
    expect_error(getURL(URL = "https://services.healthtech.dtu.dk/services/NetMHCpan-4.1/allele.list", read_method = "iamnotamethod"))
    expect_error(getURL(URL = "https://services.healthtech.dtu.dk/services/NetMHCpan-4.1/allele.list", read_method = "delim", delim = TRUE))
    expect_warning(getURL(URL = "https://services.healthtech.dtu.dk/services/NetMHCpan-4.1/allele.list", read_method = "delim", col_names = c("a","b","c","d")))
})
...
test_that("g_groups has all expected loci", {
    patterns_loci <- c("A","B","C","DPA1","DPB1","DQA1","DQB1","DRB1","DRB3","DRB4","DRB5")
...
test_that("g_groups has expected entries", {
    ...
    expect_true("01:03:01:01/01:03:01:02/01:03:02/01:287N/01:315" %in% g_group$g_group)
})
