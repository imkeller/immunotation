...
```{r}
listed.files <- list.files(system.file("extdata", ".", package = "immunotation"))
...
```{r}
result_mhc_alleles <- sapply(paste(system.file("extdata", ".", package = "immunotation"),listed.files,sep='/'),    
                             parse_json_arcasHLA_output, 
                             path_to_peptide = system.file("extdata",   "virus_example.fasta", package = "immunotation"),
                            output_bash="example.genotype.sh",
                            mhc_class_to_analyze="both")
```
...
```{r}
example1.genotype_netMHCI <- read_netMHCpan_output(system.file("extdata", "example1.genotype_netMHCI.xls", package = "immunotation"), mhc_class_to_analyze='MHC-I')
DT::datatable(example1.genotype_netMHCI, options = list(scrollX = TRUE))
```

Example for MHC-II class.

```{r}
example1.genotype_netMHCII <- read_netMHCpan_output(system.file("extdata", "example1.genotype_netMHCII.xls", package = "immunotation"), mhc_class_to_analyze='MHC-II')
DT::datatable(example1.genotype_netMHCII, options = list(scrollX = TRUE))
```

Sort only strong binders.

```{r}
DT::datatable(example1.genotype_netMHCI[example1.genotype_netMHCI$NB. >= 1,], options = list(scrollX = TRUE))
```

[One](https://www.iedb.org/epitope/194449) of strong binders could be found in The Immune Epitope Database (IEDB).

```{r}
DT::datatable(example1.genotype_netMHCI[example1.genotype_netMHCI$`core.HLA-A03:01` == 'VEDLFGANL', 10:15], options = list(scrollX = TRUE))
```
# References 
