...
antigen presentation, a high diversity of MHC genes on population level is 
...
polymorphic, which means that on a population level, there exist multiple 
variants (alleles) of each gene.

Several experimental techniques exist to identify the different MHC genes, 
alleles and protein complexes. Protein complexes for example can be identified 
by binding of subtype-specific anti-MHC antibodies. The resulting information on 
the protein complex is called MHC serotype. Moreover, MHC genes and alleles can 
...
chromosome 6. Up to date more than 28.000 different alleles have been 
...
In a deprecated naming scheme used before 2010, the components of the naming 
scheme were not separated by ":".
...
slightly different DNA sequences in coding region 

### Protein and gene groups

[*G groups*](http://hla.alleles.org/alleles/g_groups.html) are groups of 
HLA alleles that have identical *nucleotide* sequences across the exons encoding 
...
to facilitate the reporting and comparison of HLA alleles [4]. 
MAC represent groups of HLA alleles and are useful when the HLA typing is 
ambiguous and does not allow to narrow down one single allele from a list of 
alleles. The `immunotation` package provides automated access to the 
...
`immunotation` package allows the automated R access to the HLA related datasets 
in AFND.
...
genes, alleles and protein complexes. The package currently has two main 
functional modules: 
...
different species
* conversion between different levels of HLA annotation, namely specific 
functions to convert between input and output of different immunoinformatics 
tools
...
* automated access to datasets stored in the 
